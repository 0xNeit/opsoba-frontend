...
const SOBA_AS_TOKEN = new Token(ChainId.MAINNET, SOBA_AS_STRING, 18)
const BUSD_AS_TOKEN = new Token(ChainId.MAINNET, BUSD_AS_STRING, 18)
const SOBA_BUSD_LP = '0x804678fa97d91B974ec2af3c843270886528a9E6'

describe('getLpAddress', () => {
  it('returns correct LP address, both tokens are strings', () => {
    expect(getLpAddress(SOBA_AS_STRING, BUSD_AS_STRING)).toBe(SOBA_BUSD_LP)
  })
  it('returns correct LP address, token1 is string, token 2 is Token', () => {
    expect(getLpAddress(SOBA_AS_STRING, BUSD_AS_TOKEN)).toBe(SOBA_BUSD_LP)
  })
  it('returns correct LP address, both tokens are Token', () => {
    expect(getLpAddress(SOBA_AS_TOKEN, BUSD_AS_TOKEN)).toBe(SOBA_BUSD_LP)
  })
  it('returns null if any address is invalid', () => {
    ...
    expect(getLpAddress(SOBA_AS_STRING, undefined)).toBe(null)
    expect(getLpAddress(undefined, BUSD_AS_TOKEN)).toBe(null)
    expect(getLpAddress(SOBA_AS_STRING, '456')).toBe(null)
