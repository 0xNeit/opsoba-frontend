...
import { Text, Flex, Box, CloseIcon, IconButton, useMatchBreakpoints } from 'opsoba-uikit'
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://sobaswap.finance - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/sobaswap.finance)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://sobaswap.finance'}
          color={text === 'https://sobaswap.finance' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
