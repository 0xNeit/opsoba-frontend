...
interface LeaderboardRowProps {
  bidder: Bidder
  sobaPriceBusd: BigNumber
  isMobile: boolean
}

const LeaderboardRow: React.FC<LeaderboardRowProps> = ({ bidder, sobaPriceBusd, isMobile }) => {
        ...
        <Flex flexDirection="column" width="100%" justifyContent="flex-end" pr={[null, null, '24px']}>
          ...
          {sobaPriceBusd.gt(0) ? (
            <Text fontSize="12px" color="textSubtle" textAlign="right">
              ~${getBalanceNumber(amount.times(sobaPriceBusd)).toLocaleString('en', { maximumFractionDigits: 0 })}
            </Text>
          ) : (
    ...
    <Box>
      <LeaderboardContainer>
        ...
        {bidders.slice(0, visibleBidders).map((bidder) => (
          <LeaderboardRow key={bidder.account} bidder={bidder} sobaPriceBusd={sobaPriceBusd} isMobile={isMobile} />
        ))}
      </LeaderboardContainer>
